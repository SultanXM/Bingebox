...
import { getBookmarkedMovies, type StoredMovie } from "@/app/lib/userStorage";
...
export default function BookmarkedPage() {
  ...
  const handleRemove = (id: number) => {
    const updated = bookmarkedMovies.filter((m) => m.id !== id);
    setBookmarkedMovies(updated);
    localStorage.setItem("bingbox_bookmarked", JSON.stringify(updated));
  };

  const filteredMovies = bookmarkedMovies.filter(m => 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-500/20 rounded-xl flex items-center justify-center">
              <Bookmark className="h-5 w-5 text-green-500 fill-green-500" />
            </div>
            <div>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-gray-400 mb-8 max-w-sm">
              {searchQuery 
                ...
                : "Save movies and shows to watch later. They'll appear here."}
            </p>
            {!searchQuery && (
              <Link
                href="/main"
            ...
            {filteredMovies.map((movie) => (
              <div key={movie.id} className="group relative">
                ...
                <button
                  ...
                  className="absolute top-2 right-2 p-2 bg-black/70 hover:bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all duration-200"
